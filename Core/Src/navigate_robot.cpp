...
void kalibracja_robota(int givenSteps[6], int liczba_krokow_osi[5], uint8_t kalibracja_osi [5]){
	...
	givenSteps[2]=6400;
	while (givenSteps[2]!=liczba_krokow_osi[2]||givenSteps[3]!=liczba_krokow_osi[3]||givenSteps[4]!=liczba_krokow_osi[4]){}
}
...
void licz_kroki(double givenPosition[6], int givenSteps[6], double currentPosition[6])
{
    ...
    double r11 = cos(phi)*sin(theta_rotation)*cos(psi)+sin(phi)*sin(theta_rotation);
    ...
    double r23 = sin(phi)*sin(theta_rotation)*sin(psi)+cos(phi)*cos(theta_rotation);
	...
	double ax = r13*cosl(theta[0])*cosl(theta[1] + theta[2]) + r23*cosl(theta[1] + theta[2])*sinl(theta[0]) + r33*sinl(theta[1] + theta[2]);
    double ay = -r23*cosl(theta[0]) + r13*sinl(theta[0]);
    double az = -r33*cosl(theta[1] + theta[2]) + r13*cosl(theta[0])*sinl(theta[1] + theta[2]) + r23*sinl(theta[0])*sinl(theta[1] + theta[2]);
    double sz = -r32*cosl(theta[1] + theta[2]) + r12*cosl(theta[0])*sinl(theta[1] + theta[2]) + r22*sinl(theta[0])*sinl(theta[1] + theta[2]);
    double nz = -r31*cosl(theta[1] + theta[2]) + r11*cosl(theta[0])*sinl(theta[1] + theta[2]) + r21*sinl(theta[0])*sinl(theta[1] + theta[2]);
	...
	theta[5]=M_PI-theta[5];
	    ...
	    sqrt(s*s+r*r)>min_range&&sqrt(s*s+r*r)<max_range&&// warunek który musi być spełniony żeby pozycja była osiągalna ze względu na zasięg ramion robota
