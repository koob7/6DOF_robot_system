...
void XPT2046_Init(void) {
  ...
  HAL_SPI_TransmitReceive(&hspi2, (uint8_t *)XPT2046_ADDR_I, &data,
                          sizeof(data), 1000);
  HAL_SPI_TransmitReceive(&hspi2, (uint8_t *)XPT2046_ADDR_I, &data,
                          sizeof(data), 1000);
...
inline static uint16_t X(void) {
  uint16_t x;
  x = (uint16_t)remap(getRaw(XPT2046_ADDR_X), RAW_MIN_X, RAW_MAX_X, OUT_MIN_X,
                      OUT_MAX_X);
  if (XPT2046_MIRROR_X)
    x = OUT_MAX_X - x;
  if (x > OUT_MIN_X && x < OUT_MAX_X)
    return x;
  else
    return 0;
}
...
inline static uint16_t Y(void) {
  uint16_t y;
  y = (uint16_t)remap(getRaw(XPT2046_ADDR_Y), RAW_MIN_Y, RAW_MAX_Y, OUT_MIN_Y,
                      OUT_MAX_Y);
  if (XPT2046_MIRROR_Y)
    y = OUT_MAX_Y - y;
  if (y > OUT_MIN_Y && y < OUT_MAX_Y)
    return y;
  else
    return 0;
}
